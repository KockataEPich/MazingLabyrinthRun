...
public:
	...
	void add_systems(std::unique_ptr<System>&&... system) {
		([&] {
			...
			if constexpr (std::is_base_of_v<ProducerSystem, System>) { m_producer_systems.add_system(std::move(system));}
			if constexpr (std::is_base_of_v<ReactSystem, System>) { m_react_systems.push_back(std::move(system)); }
			if constexpr (std::is_base_of_v<ImpulseSystem, System>) { m_impulse_systems.push_back(std::move(system)); }
			if constexpr (std::is_base_of_v<RenderSystem, System>) { m_render_systems.push_back(std::move(system)); }
		}(), ...);
	}
